...
def get_storage() -> LocalStorage:
    ...
    This is the ONLY way to access storage across all clawdex modules.
    Ensures consistent state between recording, querying, and maintenance.

    Encryption is enabled via:
    - CLAWDEX_ENCRYPTED=true environment variable
    - CLAWDEX_PASSWORD environment variable (optional, auto-generates if not set)
    - Or by calling configure_storage() before first use
    """
    global _storage
    if _storage is None:
        encrypted = os.environ.get("CLAWDEX_ENCRYPTED", "").lower() in ("true", "1", "yes")
        password = os.environ.get("CLAWDEX_PASSWORD")
