"""Maintenance tools for Claw-dex - verify, update, stats, purge, export"""
...
async def get_stats() -> dict:
    """
    Get statistics about the Claw-dex knowledge base.
    ...
    return {
        ...
        "message": "Claw-dex statistics retrieved",
    }


async def purge_all(confirm: bool = False) -> dict:
    """
    Delete ALL Claw-dex data. This is IRREVERSIBLE.
    ...
    return {
        "success": True,
        "message": "All Claw-dex data has been permanently deleted.",
...
async def export_all(output_path: Optional[str] = None) -> dict:
    """
    Export ALL your Claw-dex data to a JSON file.

    YOUR RIGHT TO DATA PORTABILITY: This implements GDPR Article 20.
    You can export all your data at any time.

    Args:
        output_path: Where to save the export. Defaults to ~/clawdex-export-{date}.json
    ...
    export_data = {
        ...
        "metadata": {
            "format": "clawdex-export-v1",
            "description": "Complete export of Claw-dex knowledge base",
        }
    }
    ...
    if output_path is None:
        ...
        output_path = str(Path.home() / f"clawdex-export-{date_str}.json")
    ...
    return {
        "success": True,
        "message": f"Exported all Claw-dex data to {output_path}",
